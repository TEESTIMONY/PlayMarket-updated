            ...
            {users.map((user) => (
              <React.Fragment key={user.id}>
                <tr className="hover:bg-gray-50 cursor-pointer" onClick={() => toggleRowExpansion(user.id)}>
                  ...
                  <td className="px-2 md:px-6 py-2 md:py-4 text-xs md:text-sm text-gray-900 overflow-hidden text-ellipsis max-w-[80px] whitespace-nowrap" title={user.username}>{user.username}</td>
